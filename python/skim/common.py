...
def select_Jpsi_loose(events: ak.Array):
    events['selected_muons'] = events.Muon[abs(events.Muon.eta < ZHqqJpsi.eta)]
    ...
    events = events[ak.any(
          (events.dimu.mass > ZHqqJpsi.mass_Jpsi[0])
        & (events.dimu.mass < ZHqqJpsi.mass_Jpsi[1])
        , axis = -1)]
    return events
...
def select_jet_loose(events: ak.Array):
    ...
    events = events[events.selected_jets.pt[:,0] > ZHqqJpsi.pt_jet_lead]
    return events